...
// 📦 Cargar piezas personalizadas desde JSON 
fetch('./piezas.json')
  .then(res => res.json())
  .then(productos => {
    ...
    productos.forEach(producto => {
      ...
      // Si no hay precio, mostramos "Personalizado"
      const precioHTML = producto.precio
        ? `<span class="text-xl font-semibold">
             <span class="text-lime-200/70">Desde</span> $${producto.precio}
           </span>`
        : `<span class="text-xl font-semibold text-lime-300/70">
             Pieza personalizada
           </span>`

      slide.innerHTML = `
        <img src="${producto.imagen}"
             class="btn-consultar w-52 h-52 sm:w-64 sm:h-64 object-cover rounded-2xl shadow-lg">
